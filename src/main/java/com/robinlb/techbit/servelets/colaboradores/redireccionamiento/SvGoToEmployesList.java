...
public class SvGoToEmployesList extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
       ...
       */
      try {
        String num = (String) request.getSession(false).getAttribute("numNoEliminados");
       ...
       */
      try {
        lengthArray = (int) request.getSession(false).getAttribute("arrIDsForDelLength");
      ...
      /**
       * Toma el atributo de sesión para comprobar si se modifico algún registro
       * de empleado.
       */
      try {
        isMod = (String) request.getSession(false).getAttribute("empModificado");
      }
      catch (Exception b) {
